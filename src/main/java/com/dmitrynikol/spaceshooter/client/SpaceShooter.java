  ...
  implements EntryPoint
{
   ...
   */
    ...
    context.fillText( "Total time: ".concat( String.valueOf( (int) frameRate.getTotalElapsedTime() ) ).concat( " sec" ),
                      16, 58 );
    ...
    for ( Map.Entry<Powerup, Float> entry : ApplicationContext.getTempPowerups().entrySet() )
    {
      ...
      {
        context.fillText( pow.name().concat( ": " ).concat( String.valueOf( stopwatchTimer ) ), 6,
                          108 + indexCoordinate * 20 );
      }
    }
